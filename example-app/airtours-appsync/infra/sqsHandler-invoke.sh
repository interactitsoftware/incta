...
echo `node ./node-modules-layer/nodejs/node_modules/aarts-eb-dispatcher/samLocalSimulateHandler.js $EVENT_FILE_PATH` | "$SAM" local invoke $HANDLER  --event - --region ddblocal --docker-network sam-local --template template.yml --env-vars env-constants-local.json > sqsHandler-invoke.out 2>&1