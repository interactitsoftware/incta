...
import { SendWelcomeEmailItem } from "../__bootstrap/_DynamoItems"
import { AartsEvent, AartsPayload, IIdentity, IItemManager  } from "aarts-types/interfaces"


export class SendWelcomeEmailCommand extends BaseDynamoItemManager<SendWelcomeEmailItem> {
    
    /**
    * Command parameters preparation and/or validation
    */
    async *validateStart(proc: AartsPayload<SendWelcomeEmailItem>): AsyncGenerator<AartsPayload, AartsPayload, undefined> {
        ...
        if (errors.length > 0) {
            yield { resultItems: [{ message: `Start SendWelcomeEmail Failed` }, errors] }
    ...
    */
    async execute(__type: string, args: AartsEvent) : Promise<SendWelcomeEmailItem> { 
        ...
        return args.payload.arguments as SendWelcomeEmailItem
    }
}
